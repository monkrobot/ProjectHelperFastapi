...
class Tasks(Base):
    ...
    description: Mapped[list[str]] = mapped_column(String)
    creator_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("users.id"))

    executors: Mapped[list["Users"]] = relationship("Users", back_populates="tasks")
    ...
    def __str__(self) -> str:
        return f"Shopping list {self.id} {self.name}: {self.description}"
